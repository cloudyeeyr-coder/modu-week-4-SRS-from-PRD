...
| ID | 가정 | 검증 시한 |
|:---:|:---|:---|
| ASM-01 | PG사 에스크로 API가 B2B 고액 거래(건당 1억 원 이상)를 지원한다 | D-60일 |
| ASM-02 | 로봇 제조사(UR, 두산, 레인보우 등) 최소 3사가 뱃지 프로그램에 참여한다 | D-90일 LOI |
| ASM-03 | NICE 평가정보 API를 통한 SI 업체 재무 등급 조회가 법적으로 가능하다 | D-60일 법률 검토 |
| ASM-04 | 로컬 AS 사업자가 24시간 SLA에 동의하고 계약 가능하다 | D-30일 |
| ASM-05 | MVP 동시 접속 규모는 500 CCU 이내이다 | 부하 테스트 |
...
| ID | 문서명 | 설명 |
|:---:|:---|:---|
| REF-01 | `1_PRD-Robot-SI-Platform-v0.1.md` (v0.2) | 본 SRS의 원천 PRD 문서 |
| REF-02 | `02_VPS-Drafts/6_Value-Proposition-Sheet-V2(rooted).md` | 가치 제안 시트 및 JTBD 인터뷰 근거 (부록 E-1) |
| REF-03 | `01_Biz-analysis/6_TAM-SAM-SOM+MarketSegment.md` | 시장 규모 분석 (Grand View Research, 국내 로봇 산업 실태조사) |
| REF-04 | `01_Biz-analysis/2_competitents-analysis.md` | 경쟁사 분석 (마로솔 사례) |
| REF-05 | `01_Biz-analysis/1_porters-forces.md` | Porter's 5 Forces 전략 분석 |
| REF-06 | `01_Biz-analysis/4_ksf-report.md` | KSF 통합 보고서 |
| REF-07 | `01_Biz-analysis/9_aos-dos-analysis.md` | AOS-DOS 분석 |
| REF-08 | `03_PRD-Drafts/2_PRD-Quality-Review-Report.md` | PRD v0.1 → v0.2 품질 리뷰 보고서 |
| REF-09 | ISO/IEC/IEEE 29148:2018 | Systems and software engineering — Life cycle processes — Requirements engineering |
| REF-10 | 전자금융거래법 | 거래·결제 데이터 5년 보존 의무 |
...
| ID | 외부 시스템 | 인터페이스 | 프로토콜 | 제약 사항 |
|:---:|:---|:---|:---:|:---|
| EXT-01 | PG 에스크로 (토스페이먼츠/나이스) | REST API | HTTPS (TLS 1.3) | PCI-DSS Level 1, 타임아웃 10초, B2B 고액 거래(건당 1억 원+) 지원 필요 |
| EXT-02 | NICE 평가정보 (기업 신용정보) | REST API | HTTPS (TLS 1.3) | 조회 건당 과금, 일일 한도 500건, 캐시 TTL 30일 |
| EXT-03 | 카카오 알림톡 | REST API | HTTPS | 건당 10원, 초당 100건 제한 |
| EXT-04 | SMS 발송 서비스 | REST API | HTTPS | 건당 20원 이하 |
| EXT-05 | 금융 파트너 API (RaaS/리스) | REST API | HTTPS (TLS 1.3) | 응답 시간 5초 이내, 장애 시 이메일 견적 대안 경로 제공 |
...
*끝. 본 SRS v1.0은 PRD v0.2 (REF-01)를 유일한 원천(Source of Truth)으로 하여 ISO/IEC/IEEE 29148:2018 표준에 따라 작성되었습니다. Closed Beta 결과 및 실험(EXP-01~05) 수행 결과에 따라 SRS v1.1로 개정됩니다.*
