...
| ID | 제약사항 | 근거 |
|:---:|:---|:---|
| CON-11 | 모든 서비스는 Next.js (App Router) 기반의 단일 풀스택 프레임워크로 구현한다. 프론트엔드와 백엔드를 별도 분리하지 않는다 | C-TEC-001, ADR-004 |
| CON-12 | 서버 측 로직(DB 접근, API 호출 등)은 Next.js의 Server Actions 또는 Route Handlers를 사용하여 별도의 백엔드 서버 없이 구현한다 | C-TEC-002, ADR-004 |
| CON-13 | 데이터베이스는 Prisma ORM + 로컬 SQLite(개발)를 사용하고, 배포 시 Supabase(PostgreSQL)를 사용한다 | C-TEC-003 |
| CON-14 | UI 및 스타일링은 Tailwind CSS와 shadcn/ui를 사용한다 | C-TEC-004 |
| CON-15 | LLM 오케스트레이션은 별도의 Python 서버 없이 Vercel AI SDK를 사용하여 Next.js 내부에서 직접 구현한다 | C-TEC-005 |
| CON-16 | LLM 호출은 Google Gemini API를 기본으로 사용하며, 환경 변수 설정만으로 모델 교체가 가능하도록 SDK 표준 인터페이스를 준수한다 | C-TEC-006 |
| CON-17 | 배포 및 인프라 관리는 Vercel 플랫폼으로 단일화하며, CI/CD 설정 없이 Git Push만으로 배포를 자동화한다 | C-TEC-007 |
...
| ASM-05 | MVP 동시 접속 규모는 500 CCU 이내이다 | MVP D-14일 Preview 환경에서 k6를 활용한 500 CCU × 30분 부하 테스트 수행. p95 응답 시간, 에러율 확인 | 부하 테스트 (D-14) |
| ASM-06 | Vercel Pro 플랜을 사용하며, Serverless 함수 실행 시간 300초, Cron 1분 간격을 지원한다 | Vercel Pro 플랜 공식 문서에서 제한사항 확인. 테스트 프로젝트에서 300초 함수 실행 및 Cron Job 실행 검증 | D-30일 |
| ASM-07 | Supabase(PostgreSQL)가 MVP 규모의 동시 접속(500 CCU)과 데이터 규모를 지원한다 | Supabase Pro 플랜 connection pooling(PgBouncer) 설정 확인. Prisma connection pool 테스트 | D-14일 |
...
| **Next.js App Router** | Next.js 프레임워크의 파일 시스템 기반 라우팅 체계. Server Components, Route Groups, Server Actions, Route Handlers를 포함하는 풀스택 아키텍처 |
| **Server Actions** | Next.js에서 서버 측 함수를 클라이언트에서 직접 호출할 수 있는 메커니즘. 폼 처리, 데이터 변경 등에 사용 |
| **Route Handlers** | Next.js App Router에서 `/app/api/` 경로에 정의하는 서버 사이드 HTTP 엔드포인트 |
| **Prisma ORM** | TypeScript/JavaScript 기반 Object-Relational Mapping 도구. 타입 안전한 DB 접근 계층을 제공 |
| **Supabase** | PostgreSQL 기반 오픈 소스 BaaS(Backend-as-a-Service). DB, Auth, Storage, Realtime 기능 제공 |
| **Vercel AI SDK** | Vercel이 제공하는 LLM 오케스트레이션 SDK. OpenAI, Gemini 등 다양한 제공자를 단일 인터페이스로 지원 |
| **Vercel Cron** | Vercel 플랫폼에서 제공하는 스케줄 기반 작업 실행 기능. `vercel.json`에 선언적으로 정의 |
| **Vercel KV** | Vercel 플랫폼에서 제공하는 Redis 호환 키-값 저장소. 캐시, 세션 관리에 사용 |
| **shadcn/ui** | Radix UI 기반의 재사용 가능한 UI 컴포넌트 라이브러리. Tailwind CSS로 스타일링된 복사·붙여넣기 형태 제공 |
| **Sentry** | 실시간 에러 추적 및 성능 모니터링 플랫폼 |
